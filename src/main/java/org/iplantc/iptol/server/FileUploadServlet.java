 ...
 * 
 * This class extends the UploadAction class provided by the GWT Upload 
 * library. The executeAction method must be overridden for custom behavior.
 * 
 * @author sriram
 * 
 */
public class FileUploadServlet extends UploadAction {
			...
			List<FileItem> fileItems) throws UploadActionException 
	{
		...
		{
			json = invokeService(request, idFolder, idWorkspace,
					filename, bodyFile);
		}
	 ...
	 */
	private String invokeService(HttpServletRequest request, 
			String idFolder, String idWorkspace, String filename,
			String fileContents) throws UploadActionException 
	{
		String json = null;
		MultiPartServiceWrapper wrapper = createServiceWrapper(idFolder,
				idWorkspace, filename, fileContents);
	 ...
	 * @return
	 */
	private MultiPartServiceWrapper createServiceWrapper(String idFolder,
			String idWorkspace, String filename, String fileContents) 
	{
		//build our address
		// TODO: get these strings into a constants file
		// can use 'format-string' style approach: 
		//		{0}://{1}:{2}/workspaces/{3}/folders/{4}/files % http. localhost, 14444, idWorkspace, idFolder
		String address = "http://localhost:14444/workspaces/" + idWorkspace
				+ "/folders/" + idFolder + "/files";
		
		// TODO: Should there be a FileServices class that is wrapping all of this like FolderServices/etc.??? 
		//build our wrapper
	 ...
	 */
	private boolean sufficientData(String idWorkspace, String idFolder, String filename, String fileContents)
	{
