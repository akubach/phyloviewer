...
public class RawDataPanel extends ProvenanceContentPanel
{
	...
	private void doSave()
	{
		if(areaData != null)
		{
			...
			if(file != null)
			{
				...
				wait.show();
				RawDataServices.saveRawData(file.getFileId(), file.getFilename(), body,
						new AsyncCallback<String>()
						{
							...
							{
								DEErrorStrings errorStrings = (DEErrorStrings)GWT
	...
	{
		IPlantDialog dlg = new IPlantDialog(displayStrings.saveAs(), 320, new RawDataSaveAsDialogPanel(
				idWorkspace, file, areaData.getValue(), wait));
		dlg.show();
	}
