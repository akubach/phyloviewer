...
public class JobStatusPanel extends ContentPanel
{
	...
	private ToolBar buildGridToolBar()
	{
		...
		btnStart.addListener(Events.OnClick, new Listener<BaseEvent>()
		{
			...
			public void handleEvent(BaseEvent be)
			{
				...
				{
					if(JobStatusPanel.JOB_STATUS.READY.toString().equalsIgnoreCase(
							grid.getSelectionModel().getSelectedItem().get("status").toString()))
					{
		...
		btnDelete.addListener(Events.OnClick, new Listener<BaseEvent>()
		{
			...
			public void handleEvent(BaseEvent be)
			{
				if(grid.getSelectionModel().getSelectedItems().size() > 0)
				{
					MessageBox.confirm("Confirm", displayStrings.jobDeleteConfirm(),
							new Listener<MessageBoxEvent>()
							{
								...
								public void handleEvent(MessageBoxEvent be)
								{
									...
									{
										deleteJob(grid.getSelectionModel().getSelectedItem().get("id")
												.toString());
									}
								}
							});
	...
	{
		if(grid.getSelectionModel().getSelectedItems().size() <= 0
				|| grid.getSelectionModel().getSelectedItem().get("status").equals(
						JobStatusPanel.JOB_STATUS.READY)
				|| grid.getSelectionModel().getSelectedItem().get("status").equals(
						JobStatusPanel.JOB_STATUS.COMPLETED))
		{
	...
	private void startJob(final String jobid)
	{
		...
		JobServices.runContrastJob(jobid, new AsyncCallback<String>()
		{
			...
			{
				org.iplantc.de.client.ErrorHandler.post(errorStrings.runJobError());
			}
	...
	private void deleteJob(String jobid)
	{
		...
		JobServices.deleteContrastJob(jobid, new AsyncCallback<String>()
		{
			...
			public void onFailure(Throwable caught)
			{
				org.iplantc.de.client.ErrorHandler.post(errorStrings.deleteJobError());
	...
	private void getContrastJobs()
	{
		JobServices.getContrastJobs(workspaceId, new AsyncCallback<String>()
		{
			...
			public void onFailure(Throwable caught)
			{
				org.iplantc.de.client.ErrorHandler.post(errorStrings.getJobsError());
					...
					for(int i = 0;i < jobinfos.length();i++)
					{
						if(jobid.equals(jobinfos.get(i).getId().toString()))
						{
							if(jobinfos.get(i).getStatus().equals(
									...
									|| jobinfos.get(i).getStatus().equals(
											JobStatusPanel.JOB_STATUS.COMPLETED.toString()))
							{
								cancelTimer();
								pushResultFiletoWorkSpace(jobinfos.get(i).getName());
							}
							d = new Date(Long.parseLong(jobinfos.get(i).getCreationDate()));
							j = new Job(jobinfos.get(i).getId(), jobinfos.get(i).getName(),
									d.toString(), jobinfos.get(i).getStatus());
		...
		FolderServices.getListofFiles(workspaceId, new AsyncCallback<String>()
		{
			...
			public void onSuccess(String result)
			{
				...
				for(int i = 0;i < fileinfos.length();i++)
				{
					...
					if(info.getName().equals(successFileName) || info.getName().equals(errorFileName))
					{
						FileUploadedEvent event = new FileUploadedEvent("", info, null);
		...
		FolderServices.getListofFiles(workspaceId, new AsyncCallback<String>()
		{
			...
			public void onSuccess(String result)
			{
				...
				for(int i = 0;i < fileinfos.length();i++)
				{
					...
					if(info.getName().equals(successFileName) || info.getName().equals(errorFileName))
					{
						// address = "http://" + Window.Location.getHostName() +
						// ":14444/files/" + info.getId() + "/content";
						address = Window.Location.getProtocol() + "://" + Window.Location.getHost()
								+ Window.Location.getPath() + "files/" + info.getId()
								+ "/content.gdwnld";
						break;
					}
				}
				...
				{
					// TODO displayStrings is not where error messages go, move to
					// errorStrings
