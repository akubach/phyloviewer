...
public class FileUploadPanel extends IPlantDialogPanel
{
	...
	public FileUploadPanel(HashMap<String,String> hiddenFields, String servletActionUrl,
			UploadCompleteHandler handler)
	{
		...
		fupload.addChangeHandler(new ChangeHandler()
		{
			 ...
			 * 
			 * This is only fired when an actual file is selected, not merely when the
		...
		FolderServices.getListofFiles(idWorkspace, new AsyncCallback<String>()
		{
			...
			public void onSuccess(String result)
			{
				...
				for(int i = 0,len = fileinfos.length();i < len;i++)
				{
					...
					{
						dupeFound = true;
						// when a duplicate is found, ask the user if they want to
						// overwrite it
