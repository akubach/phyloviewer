...
/**
 * Provides a user interface for editing trait data.  
 */
public class TraitEditorPanel extends ContentPanel
{
	// TODO: do we want this at package scope? 
	...
	private ToolBar buildToolBar()
	{
		...
		save.addSelectionListener(new SelectionListener<ButtonEvent>()
		{
			...
			public void componentSelected(ButtonEvent ce)
			{
				...
				TraitDataJsonGen gen = new TraitDataJsonGen(store.getModels(), parser.getHeader()
						.isArray());
	...
	private void loadData()
	{
		...
		for(int i = 0;i < headers.size();i++)
		{
			...
			column = new ColumnConfig(TraitDataJsonParser.trim(obj.get("id").toString()),
					TraitDataJsonParser.trim(obj.get("label").toString()), 100);
			...
			{
				type.addField(TraitDataJsonParser.trim(TraitDataJsonParser
						.trim(obj.get("id").toString())), TraitDataJsonParser.makeUpEmptyHeader(i));

			}
			else
			{
				type.addField(TraitDataJsonParser.trim(TraitDataJsonParser
						.trim(obj.get("id").toString())), TraitDataJsonParser.trim(obj.get("label")
						.toString()));
			}
		...
		MemoryProxy<ModelData> proxy = new MemoryProxy<ModelData>(reader.read(null, parser.getData()
				.toString()));
