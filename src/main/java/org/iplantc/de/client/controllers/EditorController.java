...
public class EditorController
{
	...
	private void removeFileWindow(ProvenanceWindow in)
	{
		if(in != null)
		{
			// since minimized is really hidden... we need to check or else the hide event
			// is never picked up
			// the result is that the user can delete a 'minimized' provenance window and
			// it will not be removed
			// from the toolbar. The solution is to show the window first and then hide.
			// If this causes visual
			// anomalies on slower machines, we will likely have to fire an event to have
			// the perspective remove
