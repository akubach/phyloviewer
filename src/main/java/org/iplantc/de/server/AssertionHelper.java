...
/**
 * Provides utility functionality to aide in the creation and processing of SAML
 * Assertions
 * 
 * @author lenards
 * 
...
public class AssertionHelper
{
	 ...
	 * @param privKey private key to use, with X509 certificate, when signing the
	 *            assertion
			...
			PrivateKey privKey, PublicKey encryptKey) throws Saml2Exception, MarshallingException
	{
		...
		// Sign assertion
		builder.signAssertion(cert, privKey, SignatureConstants.ALGO_ID_SIGNATURE_RSA_SHA1);
	 ...
	 * @param privKey private key to use, with X509 certificate, when signing the
	 *            assertion
	 * @param encryptKeyPair key-pair containing the public key to use when encrypting the
	 *            assertion
	 * @return a base64 encoded version of the encrypted SAML Assertion
	 * @throws Saml2Exception
	 * @throws MarshallingException
	 ...
	 * @throws Saml2Exception
	 */
	private static void includeAttributes(HttpServletRequest request, AssertionBuilder builder)
			throws Saml2Exception
	{
		...
		for(String attrName : DESecurityConstants.getLocalAttributeNames())
		{
			...
			if(value != null && !value.toString().isEmpty())
			{
				...
				attributeStatementBuilder.addStringAttribute(attrDisplayName, ATTRIBUTE_URL, value
						.toString());
			}
		}
	}
	...
	/**
	 * Gets the display name for the attribute. For our purposes, the display name is the
	 * same as the local attribute name, but with the "shibboleth" prefix removed.
	 * 
	 ...
	 * @throws MarshallingException
	 */
	public static String encryptAssertion(Assertion assertion, PublicKey encryptionPublicKey)
			throws MarshallingException
	{
