...
/**
 * An authentication filter that allows or denies access based on the presence or absence
 * of a valid SAML assertion that is known by the local Shibboleth service provider. Our
 * login page is the only page that is guarded by Shibboleth; when it is accessed it
 * stores assertion URLs in a session property. If this property is defined and at least
 * one of the URLs stored in this property produces a SAML assertion then the user is
 * authenticated. Otherwise, the user is not authenticated.
 * 
 * Because this class is used to secure pages that are normally only called via AJAX,
 * performing a redirection at this point is inappropriate (redirecting an AJAX requests
 * always results in the AJAX connection throwing an exception). Because of this, we
 * always block the connection and let the client handle the redirection.
 * 
...
public class AuthFilter implements Filter
{
	...
	/**
	 * A certificate trust store that trusts all certificates. This trust store will be
	 * used exclusively for connections that have been forced to point to the loopback
	 * interface, so we can be sure that they're secure. Important note: be sure to
	 * reinstall the default trust manager after the connection is established.
	 */
	...
	/**
	 * A host name verifier that accepts all host names, no matter what's in the
	 * certificate. Important note: be sure to reinstall the default host name verifier
	...
	/**
	 * Determines whether or not the user is authenticated. The user is authenticated if
	 * our Shibboleth service provider knows of at least one session that the user is
	 * associated with.
	 * 
	 ...
	 * 
	 * @param urlString the string representation of the URL to use when attempting to
	 *            fetch the assertion.
	...
	/**
	 * Obtains a URL connection for which SSL certificates and host names are not
	 * verified. It is safe to do so in this case because we're forcing the URL to point
	 * to the loopback interface, meaning that someone would have had to infiltrate the
	 * discovery environment server to stage an attack using any of these URLs. Note that
	 * this method changes the default trust store and host name verifier for SSL
	 * connections. It is essential that the original trust store and host name verifier
	 * are restored before this method exits.
	 * 
	...
	/**
	 * Builds a URL based on the given URL string that points to the loopback interface.
	 * To form this URL, the host name or IP address in the original URL string is
	 * replaced with the loopback IP address (127.0.0.1).
	 * 
	 * @param originalUrlString the URL string that was provided to us by Shibboleth.
	 * @return a URL that is based on the original URL string but contains an IP address
	 *         of 127.0.0.1.
