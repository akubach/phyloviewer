...
/**
 * When a file is updated, the Mule message contains the Content-Type and the payload is
 * the multipart containing the file contents. Use the HTTP MultiPartParser to extract the
 * file contents.
 * 
...
public class ParseUpdatePayload extends AbstractMessageAwareTransformer
{
	...
	public Object transform(MuleMessage message, String encoding) throws TransformerException
	{
		...
		if(!(message.getPayload() instanceof String))
		{
			throw new TransformerException(MessageFactory
					.createStaticMessage("Update Payload is not a string"));
		}
		...
		{
			throw new TransformerException(MessageFactory
					.createStaticMessage("No content-type in multi-part file update"));
		}
		...
		/*
		 * Construct the new payload (file contents, filename, workspaceId, optional
		 * folderId).
		 */
	...
	/**
	 * Get the Content-Type from the message. Handle the case where the property name can
	 * be any case.
	 * 
	 * @param message
	 * @return
